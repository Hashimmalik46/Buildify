...
const HRDashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      const [{ data, error }, { data: roadmapData, error: roadmapError }] = await Promise.all([
      ...
      if (data && !error && !roadmapError) {
        ...
        const totalScore = employees.reduce((sum, employee) => sum + (employee.score || 0), 0);
        const avgScore = totalEmployees > 0 ? Math.round(totalScore / totalEmployees) : 0;
  ...
  const handleGenerateTrends = async () => {
    ...
    if (!description) {
      setFeedback({ type: 'error', text: 'Enter the current industry priorities before generating trends.' });
      return;
    }
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '24px', marginBottom: '32px' }}>
        <KpiCard title="Total Employees" value={stats.totalEmployees.toLocaleString()} icon={<Users />} color="#00d2ff" />
        <KpiCard title="Avg AI Score" value={`${stats.avgScore}%`} icon={<TrendingUp />} color="#34d399" />
        <KpiCard title="Active Roadmaps" value={stats.roadmapCount.toString()} icon={<Target />} color="#eab308" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: '24px' }}>
        <div className="glass-panel" style={{ padding: '24px' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginBottom: '16px' }}>
            Describe the company&apos;s current priorities and we&apos;ll rescore the workforce against them.
          </p>
          <textarea
          ...
          {feedback && (
            <div
              style={{
                ...
                padding: '14px 16px',
                borderRadius: '12px',
                background: feedback.type === 'error' ? 'rgba(239,68,68,0.1)' : 'rgba(52,211,153,0.1)',
        ...
        <div className="glass-panel" style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ padding: '16px', background: 'rgba(58, 123, 213, 0.1)', borderLeft: '4px solid #3a7bd5', borderRadius: '4px 12px 12px 4px' }}>
              ...
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>The current average AI assessment score is {stats.avgScore} out of 100.</p>
            </div>
            <div style={{ padding: '16px', background: 'rgba(234, 179, 8, 0.1)', borderLeft: '4px solid #eab308', borderRadius: '4px 12px 12px 4px' }}>
              <h4 style={{ color: '#eab308', marginBottom: '8px', fontSize: '14px' }}>Roadmap Coverage</h4>
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>{stats.roadmapCount} roadmap assignment(s) are currently active across the workforce.</p>
            </div>
          </div>
        </div>
      </div>
