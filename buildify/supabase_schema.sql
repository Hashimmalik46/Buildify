...
CREATE TABLE profiles (
  ...
  -- The Core Grading Metric (AI-driven based on Market Fit)
  market_fit_score DECIMAL(4,2) DEFAULT 0, -- AI-calculated score (1-10)
  grade_level INTEGER CHECK (grade_level >= 1 AND grade_level <= 10), -- Final AI Level (1-10)
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, email, first_name, last_name, role, market_fit_score, grade_level, experience_years, department, skills, bio)
  VALUES (
    new.id, 
    new.email, 
    ...
    (new.raw_user_meta_data->>'marketFitScore')::DECIMAL,
    (new.raw_user_meta_data->>'gradeLevel')::INTEGER,
